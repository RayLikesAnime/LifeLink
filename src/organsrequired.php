...
$organs_query = mysqli_query($conn, "SELECT DISTINCT organ FROM organ_required");
...
<body>
    ...
    <div class="overflow-x-auto mr-8">
        ...
        if (isset($_GET['organ']) && !empty($_GET['organ'])) {
            ...
            $query = "SELECT * FROM organ_required NATURAL JOIN patient WHERE organ = '$organ' ";
            ...
            // $organ = mysqli_real_escape_string($conn, $_GET['organ']);
            $query = "SELECT * FROM organ_required NATURAL JOIN patient";
			$result = mysqli_query($conn, $query);
			$num_rows = mysqli_num_rows($result);
			if ($num_rows > 0) {
				// Display patients in a table
